...
def bede(img, layer, augmLum, augmSat, nbBilatBord, methodeDesatBord, supprParasite, forceTrait, nbBilatFinal, antialias, antialiasChaque, despeckle):
	...
	pdb.plug_in_gmic(img, layer, 1, '-gimp_curves_interactive 7,0,1,7,0,0,'+pointLum+',100,100,-1,0,0,'+pointSat+',100,100,-1,0,0,100,100,-1,0,0,100,100,-1,')
	
	pdb.gimp_image_set_active_layer(img, layerCopy)
	pdb.plug_in_gmic(img, layerCopy, 1, '-gimp_smooth_nlmeans 4,4,10,3,0,0,24,0')
	
	if antialiasChaque:
		for i in range(int(nbBilatBord)):
			pdb.plug_in_gmic(img, layerCopy, 1, '-gimp_smooth_bilateral 10,7,1,0,0')
			pdb.plug_in_gmic(img, layerCopy, 1, '-gimp_smooth_antialias '+str(antialias)+',10,0,0')
	else:
		pdb.plug_in_gmic(img, layerCopy, 1, '-gimp_smooth_bilateral 10,7,'+str(nbBilatBord)+',0,0')
	...
	if antialiasChaque:
		for i in range(int(nbBilatFinal)):
			pdb.plug_in_gmic(img, res, 1, '-gimp_smooth_bilateral 10,7,1,0,0')
			pdb.plug_in_gmic(img, res, 1, '-gimp_smooth_antialias '+str(antialias)+',10,0,0')
	else:
		pdb.plug_in_gmic(img, res, 1, '-gimp_smooth_bilateral 10,7,'+str(nbBilatFinal)+',0,0')
		pdb.plug_in_gmic(img, res, 1, '-gimp_smooth_antialias '+str(antialias)+',10,0,0')
